...
- Máximo 2 oraciones. Texto plano. Sin emojis, asteriscos, guiones, markdown ni listas. Los eventos son del USUARIO (usa "tu/tienes", nunca "mi/tengo").`
  ...
  // Reemplazo de voseo/argentino por español neutro. Se aplica SIEMPRE al reply.
  function neutralizeSpanish(text) {
    if (!text) return text
    const pairs = [
      // imperativos voseo → imperativos tú
      ...
      [/\bcreés\b/gi, 'crees'], [/\bcreés\b/gi, 'crees'],
      // modismos
      [/\bdale\b/gi, 'claro'],
      [/\bche\b/gi, ''],
      [/\bacá\b/gi, 'aquí'],
